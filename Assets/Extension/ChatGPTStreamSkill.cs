...
public class ChatGPTStreamSkill : SkillBase
{
    ...
    private void Start()
    {
        ...
        var processingAnimation = new List<ChatdollKit.Model.Animation>();
        processingAnimation.Add(new ChatdollKit.Model.Animation("BaseParam", 3, 0.3f));
        processingAnimation.Add(new ChatdollKit.Model.Animation("BaseParam", 3, 20.0f, "AGIA_Layer_nodding_once_01", "Additive Layer"));
        ...
        var animationOnStart = new List<ChatdollKit.Model.Animation>();
        animationOnStart.Add(new ChatdollKit.Model.Animation("BaseParam", 6, 0.5f));
        animationOnStart.Add(new ChatdollKit.Model.Animation("BaseParam", 10, 3.0f));
    ...
    protected async UniTask ParseAnimatedVoiceAsync(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (splittedBuffer.Count() > splitIndex + 1 || isResponseDone)
            {
                ...
                {
                    MessageWindow.Show(responseText);
                }
            }
    ...
    protected class ChatGPTStreamDownloadHandler : DownloadHandlerScript
    {
        ...
        protected override bool ReceiveData(byte[] data, int dataLength)
        {
            if (data == null || data.Length < 1) {
                Debug.Log("ChatGPTStreamDownloadHandler: ReceiveData - received a null/empty buffer");
                return false;
            }
